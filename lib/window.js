...
module.exports.Window = class Window {
  ...
  newWindowCommand() {
    let path       = this._getWindowRootPath();
    this.root      = path;
    let winTarget  = this._getWindowTarget();
    this.winTarget = winTarget;
    let nameOpt    = this.tmuxWindowNameOption();
    return `${this.loadedData.tmux} new-window ${path} -t ${winTarget} ${nameOpt}`;
  }
  ...
  _buildWindowCommands() {
    ...
    // it has an array with consectuive commands
    ...
    for(var key in this.windowYaml) {
      ...
      if(typeof command === "string") {
        return(
          [`${this.tmuxWindowCommandPrefix()} ${shellwords.escape(command)} C-m`]
        );
