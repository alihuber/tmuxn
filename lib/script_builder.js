...
module.exports.ScriptBuilder = class ScriptBuilder {
  ...
  buildScript () {
    ...
    let isSessionStarted = new SessionChecker(this.tmuxBinCommand, this.loadedData.name).sessionStarted();
    if (isSessionStarted) {
  ...
  _getBaseIndex () {
    return new TmuxOptions(this.tmuxBinCommand).options()['base-index'];
  }

  _getPaneBaseIndex () {
    return new TmuxOptions(this.tmuxBinCommand).options()['pane-base-index'];
  }
  ...
  _onProjectRestartLine () {
    return 'on_project_restart' in this.loadedData ? this.loadedData.on_project_restart : '';
  }

  _onProjectStartLine () {
    return 'on_project_start' in this.loadedData ? this.loadedData.on_project_start : '';
  }
  ...
  _startupPane () {
    let pane = 'startup_pane' in this.loadedData ? this.loadedData.startup_pane : this.loadedData.paneBaseIndex;
    let startupWindow = this._startupWindow();
    return `${startupWindow}.${pane}`;
  }

  _startupWindow () {
    let window = 'startup_window' in this.loadedData ? this.loadedData.startup_window : this.loadedData.baseIndex;
  ...
  _preLine () {
    return 'pre' in this.loadedData ? this.loadedData.pre : '';
  }

  _onProjectFirstStartLine () {
    return 'on_project_first_start' in this.loadedData ? this.loadedData.on_project_first_start : '';
  }

  _postLine () {
    return 'post' in this.loadedData ? this.loadedData.post : '';
  }

  _onProjectExitLine () {
    if ('attach' in this.loadedData && this.loadedData.attach == false) {
      return 'on_project_exit' in this.loadedData ? this.loadedData.on_project_exit : '';
    } else {
      return '';
    }
  }
