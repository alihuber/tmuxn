...
module.exports.createNewProjectWithName = projectName => {
  ...
  let sampleFile = fs.readFileSync(samplePath, 'utf-8').replace(/\$\$name\$\$/g, projectName);
...
module.exports.parseYamlByFileName = (projectFileName, root, server, web, pal) => {
  let configPath = projectFileName;
  let fileContent = fs.readFileSync(configPath, 'utf8');
  fileContent = fileContent
    .replace(/<%= @settings\["root"\] %>/g, root)
    .replace(/<%= @settings\["server"\] %>/g, server)
    .replace(/<%= @settings\["web"\] %>/g, web)
    .replace(/<%= @settings\["pal"\] %>/g, pal);
  let loadedData = yaml.safeLoad(fileContent);
  return loadedData;
};
