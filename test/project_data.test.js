...
const multiplePaneLoadedYaml = { name: 'test',
  root: '/Users/foo',
  windows: 
...
const multiplePaneWithPreWindowYaml = { name: 'test',
  root: '/Users/foo',
  pre_window: 'pre-window',
  windows: 
   [ { server: 'node app.js', synchronize: 'after', root: "/Users/bar" },
...
const expectedProjectLines = ["TMUX= tmux new-session -d -s test -n server",
...
describe('project data class', () => {
  ...
  test('it has a constructor', () => {
    expect(new ProjectData(loadedYaml))
      .toMatchObject(expectedProjectData);
  });

  test('it returns project lines', () => {
    expect(new ProjectData(loadedYaml).getProjectLines())
      .toMatchObject(expectedProjectLines);
  });

  test('it loads multiple pane configs', () => {
    expect(new ProjectData(multiplePaneLoadedYaml).getProjectLines())
      .toMatchObject(expectedMultiplePaneProjectLines);
  });

  test('it loads multiple pane with pre-window configs', () => {
    expect(new ProjectData(multiplePaneWithPreWindowYaml).getProjectLines())
      .toMatchObject(expectedMultiplePanePreWindowLines);
  });
});
